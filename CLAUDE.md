...
- Alert sound: [Sounds/alert.wav](Sounds/alert.wav) is bundled as MSBuild `Content` and copied to `Resources\alert.wav` next to the exe (see the `<Content Include="Sounds\alert.wav">` item in the csproj). [SoundService.ResolveSoundPath](Services/SoundService.cs) looks up in order: `AppSettings.CustomSoundPath` → `Resources\alert.wav` next to the exe → legacy `alert.wav` next to the exe → falls back to `SystemSounds.Exclamation`. Keep that fallback chain intact. `PlayAlert` is throttled to once per 800ms. There is no longer a `WavGenerator` (an earlier version generated the WAV at first run; it was removed in favor of shipping the file).
